...
from ascendc_multi_turn.knowledge_v2.router import KnowledgeRouterV2, SnapshotView
...
class ProjectKnowledgeTests(unittest.TestCase):
    def test_project_cards_have_evidence_and_route_host_failure(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            snapshot = build_snapshot(source=source, output=root / "store", version="8.5.0")
            view = SnapshotView(snapshot)
            bundle = KnowledgeRouterV2(view).route(KnowledgeContext(
            ...
            self.assertTrue((snapshot / "raw/project_knowledge.json").is_file())
            self.assertNotIn("Host", view.symbols)

    def test_missing_project_evidence_rejects_snapshot(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with self.assertRaisesRegex(ValueError, "evidence is missing"):
                build_snapshot(source=source, output=Path(temporary) / "store", version="8.5.0")
