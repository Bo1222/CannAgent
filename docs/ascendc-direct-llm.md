...
3. 首轮调用知识路由器查看带唯一 `doc_id` 的完整 API 清单，建立最多 8 篇文档的纯 AscendC 任务级工作集；直接流程只允许 AscendC API 页、运行时头文件事实、核心规则和纯 AscendC 专项资料，不允许选择 `dsl2Ascendc_*` 文档。稳定的后续轮次直接复用，不再调用路由模型。
